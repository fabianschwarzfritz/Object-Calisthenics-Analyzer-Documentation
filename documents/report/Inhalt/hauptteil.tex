...
In the graphic, all node causing it's children to be indent are marked black. All nodes that are not causing it's children to indent marked in blue. 
...
[font=\small, edge from parent, 
    ...
    child { node { Method Declaration }
        ...
        child { node [ draw=blue!60 ] { Indenting }
            child { node { dNon indenting } }
            child { node { eNon indenting } }
        }
        child { node [ draw=blue!60 ] { Indenting }
        	%edge from parent node[ left=.5em,draw=red!60] { }
            child { node { Indenting} [ draw=blue!60 ] 
            %	edge from parent node[ left=.5em,draw=red!60] { }
            }
            child { node { Non indenting } }
     	}
...
Instead of checking the length of each path from downward starting with the method node ending at the leaf node, the algorithm goes upward. 
...
The simple structure of a subtree is shown in \ref{ast:rule2}. The else body, which is not allowed, is marked in red. 
...
The list contians a short explanation why the listed statements are "top level statements".
\\
...
// according to JLS spec
 - one line has more than one dot.
 - what is "one line": one line is an statement. 
 - The easist ist the expresion statement, wrapping an expression into a statement. exampel. But it also is a for, a while, a switch, and so on. 
 - This includes while, dowhile, try, block, assert and many more multi line statements. This is each statement that contains an expression. "Call these statements line statements"
 - list all statements
 - list all statements that contain an expression as child.
 - there are a variety of expression and some of them contain a dot. 
 - These are herausgefunden manually according to the spec: ...list...
 - the others are ... list...
 - > The given statements are noly allowed to have a pecific amount of expressions that contain a dot (two)
  - > make clear again: These statement are the "line statement". They represent the statemts that appear in "one line"
  - > In implenentation: Thhe line statement are visited

 - The algorithm has a top down approach: A "line statements" and extracts all expressions. Expressions do not contain further statements! , and searches for all expressions containing a dot iterating greedy strategy throught the AST
 - LIsten on all statement that consist of an expression.
 - eeach expression can contain further expressions.
 - however has to handle each type of expression differently, because the methods to access sub expressions s different. --> Different methods. (give 2 examples). Other words: Many special expressions contain further expressions as child nodes. The access on these expressions is type dependant.
 - Greedy iteration through the tree of expressions of the given "line statement" counting the number of expression consisting of a dot.
  - number of dots is the result.

 indicate error if necessary.

 Algortithm does not satisfy the rule 100 percent: Definition of aline can vary. (example system out println). Even better: the algorithm checks on a more sophisticated level: statement level. In essay: probably meant is exactly this. 

--> Very good solution, precise 
% - what is the concrete violation?
% IF ALGORITHM FOUND
% - what is the algorithm to determine it? 
% - assume typical structure
...
\subsection*{Validation of Rule 5}
 - no generel rule
  - restriction in length: maximum 15 chars
   - camel case in classnames: one camel (dromedar ;))
   - camel cas in types, because first letter capital
   - show regex
   applied on each Type (TypeDeclaration)
