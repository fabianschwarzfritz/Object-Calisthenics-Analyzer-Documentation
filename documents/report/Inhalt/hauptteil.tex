...
According to Jeff Bay the change from programming in an procedural way towards programming object oriented style is hard to learn. "Transitioning from procedural development to object-oriented design requires a major shift in thinking that is more difficult than it seems" \ref[p. 70]{bay2008}. 
%TODO 2 te quelle die zeigt, dass das lernen von oo von procedural schwer ist
According to Jeff Bay, however, the procedural way of thinking is still in the mind of many developers. "Transitioning from procedural development to object-oriented design requires a major shift in thinking that is more difficult than it seems" \ref[p. 70]{bay2008}. %TODO hier 2 te qeulle und noch eien satz? sosnt halber absatz zitiert...
"Many developers assume they’re doing a good job with OO design, when in reality they’re unconsciously
stuck in procedural habits that are hard to break" \ref[p.70]{bay2008}.
...
He states that in %\ref jeffbay!!!!
dann qualities beschreiben

\subsection{Cohesion}
Cohesion describes how focused a class is. Does the module does only do what is supposed to do - and delegate everything that is not exactly part of the unit - then the module's cohesion is high. Furthermore cohesion describes how strong classes belong together? If there are two classes with a very similar purpose, then the focus of the two classes are high - but they are implementing quite similar behavior. Then the classes have a low cohesion. 
Vice versa, if there are two classes with two clearly specified and minimalist behaviors. And the behavior of the classes can be clearly separated from each other, then the classes' cohesion is high. The cohesion is high when "modules are grouped  because they all contribute to a single, well defined task" \ref{wiki:cohesion} %TODO wiki cohesion link and reference
High cohesion is considered better, because %TODO
...
When this is not the case, the classes are strongly coupled. The "lack of coupling means that the elements of our system are better isolated from each other and from change" \ref{wiki:loosecoupling}. 
% TODO quote is wrong!
...
Martin describes the \acf{DIP} in \ref{cc} as following: 
% TODO insert quot loosely coupling 
...
Well encapsulated classes "restict component's access"\ref{}. Furthermore, well encapsulated classes "bundle [...] methods that implement operations on data"\ref{}. The date is then "hidden behind the specified methods"\ref{}.
