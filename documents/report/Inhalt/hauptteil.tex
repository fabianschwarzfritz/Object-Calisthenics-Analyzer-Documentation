...
All statements that have indented subnodes can be simply read up in the \acf{JLS}. In the following, these nodes are listed. In this report, the classname of the object representation of the Eclipse \acf{JDT} framework is used. The names are similar to the names used in the \acf{JLS}: DoStatement, EnhancedForStatement, ForStatement, IfStatement, SwitchStatement, WhileStatement, and TryStatement. 
\\
...
The rule prohibits the use of the else keyword. According to the \acf{JLS}, the else statement is the body part of the if statement. In the \acf{AST} representation, the if statement has several child nodes. The two mandatory child nodes of the if statement are the condition and the body. The else body is optional. 
\\

The rule prohibits the use of the else keyword. Within the \acf{AST} structure the else keyword is represented by an object, which is a child node of the if statement.
\\
...
All in all the resulting prototype is working fine - the given requirements are fullfilled, exept for RQA5. The given requirements RQA3,RQA4 and RQA8 are fullfilled but could still be improved.
\\
